...
export default function RoutePage() {
  ...
  const getRouteColor = (type: TransitMode, id?: string) => {
    switch (type) {
      ...
      case 'subway': return id === 'BSL' ? 'bg-mode-subway-bsl' : 'bg-mode-subway-mfl';
  ...
  const getModeIcon = (type: TransitMode) => {
    switch (type) {
      case 'bus': return Bus;
      case 'trolley': return TramFront;
      case 'subway': return Train;
      case 'regional_rail': return Train;
    ...
    <>
      ...
      <div className="max-w-lg mx-auto px-4 py-6 space-y-6">
        ...
        <div className="card p-6">
          <div className="flex items-start justify-between gap-4 mb-4">
            <div className="flex items-center gap-4">
              <div className={`${getRouteColor(route.routeType, route.routeId)} w-16 h-14 rounded-xl flex items-center justify-center`}>
                <span className="text-white font-mono font-bold text-2xl">{route.routeShortName}</span>
              </div>
              <div>
                <h1 className="text-xl font-bold text-text-primary">{route.routeLongName}</h1>
          ...
          <div className="flex items-center gap-3 text-sm">
            <span className="text-text-secondary">{route.directions[0]?.destinationName}</span>
            <ArrowRight className="w-4 h-4 text-text-muted" />
            <span className="text-text-secondary">{route.directions[1]?.destinationName || route.directions[0]?.destinationName}</span>
          </div>
        </div>
            ...
            <div className="flex items-center gap-3 p-4 rounded-xl bg-urgent/10 border border-urgent/20">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-sm font-medium text-urgent">
                  {alerts.length} alert{alerts.length > 1 ? 's' : ''} on this route
                </p>
        ...
        {route.directions.length > 1 && (
          <section>
            <h2 className="text-sm font-semibold text-text-muted uppercase tracking-wider mb-3">Direction</h2>
              ...
              {route.directions.map((direction, index) => (
                ...
                >
                  <div className="flex items-center gap-2 mb-1">
                    <ArrowRight className={`w-4 h-4 ${selectedDirection === index ? 'text-septa-gold' : 'text-text-muted'}`} />
                    <span className="text-sm text-text-muted">{direction.directionName}</span>
                  </div>
        ...
        <section>
          ...
          {routeStops.length > 0 ? (
              ...
              {routeStops.map((stop, index) => (
                ...
                >
                  ...
                  `}>
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="font-medium text-text-primary truncate">{stop.stopName}</p>
                      <p className="text-sm text-text-muted font-mono">#{stop.stopId}</p>
                    </div>
            ...
            <div className="card p-8 text-center">
              ...
              <p className="text-text-secondary">Stop information not available</p>
            </div>
          )}
        </section>
          ...
          <div className="card p-4 space-y-3">
            <div className="flex justify-between items-center">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-text-primary font-medium">{getModeLabel(route.routeType)}</span>
              </div>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-text-muted">Route</span>
              <span className="font-mono font-bold text-text-primary">{route.routeShortName}</span>
            </div>
            <div className="flex justify-between items-center">
