...
import { MapPin, Locate, RefreshCw, ChevronRight, Navigation2 } from 'lucide-react';
...
import { getNearbyStops, getRealTimeArrivals, SEPTA_ROUTES } from '@/lib/septa-api';
...
export default function NearbyPage() {
  ...
  const [stopsWithArrivals, setStopsWithArrivals] = useState<StopWithArrivals[]>([]);
  const [isRefreshing, setIsRefreshing] = useState(false);
  const [lastUpdated, setLastUpdated] = useState<Date | null>(null);

  // Fetch stops and their arrivals
  const fetchStopsAndArrivals = useCallback(async (showRefresh = false) => {
    if (!location) return;
    ...
    setStopsWithArrivals(nearbyStops.map(stop => ({
      ...stop,
      arrivals: [],
    ...
    const updatedStops = await Promise.all(
      nearbyStops.map(async (stop) => {
        try {
          ...
          const arrivals = (response.data || []).slice(0, 3).map(arrival => {
            const route = SEPTA_ROUTES.find(r => r.routeId === arrival.routeId);
            return { ...arrival, routeType: route?.routeType };
          });
      ...
      <div className="max-w-lg mx-auto px-4 py-6 space-y-6">
        ...
        <div className="card p-4">
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1">
              {location ? (
                <>
                  <p className="font-semibold text-text-primary">Location enabled</p>
                  <p className="text-sm text-text-muted">
                    Showing stops within {radius >= 1000 ? `${radius / 1000}km` : `${radius}m`}
                  </p>
                </>
              ) : permission === 'denied' ? (
                <>
                  <p className="font-semibold text-text-primary">Location denied</p>
                  <p className="text-sm text-text-muted">Enable in browser settings</p>
                </>
              ) : (
                <>
                  <p className="font-semibold text-text-primary">Location disabled</p>
                  <p className="text-sm text-text-muted">Enable to find nearby stops</p>
                </>
              )}
            </div>
          ...
          <>
            ...
            {stopsWithArrivals.length === 0 ? (
              <div className="card p-8 text-center">
                ...
                <p className="font-semibold text-text-primary mb-2">No stops nearby</p>
                      ...
                      <div className="p-4 hover:bg-bg-highlight transition-colors">
                        <div className="flex items-start justify-between mb-3">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <p className="font-semibold text-text-primary">{stop.stopName}</p>
                              <p className="text-sm text-text-muted">{stop.distanceText} away</p>
                            </div>
                          </div>
        ...
        {!location && permission !== 'denied' && (
          <div className="card p-8 text-center">
            ...
            <p className="font-semibold text-text-primary mb-2">Find stops near you</p>
            <p className="text-sm text-text-secondary mb-6">
              Enable location access to discover nearby SEPTA stops and see real-time arrivals
            </p>
            <button
              onClick={requestLocation}
              disabled={isLoadingLocation}
