...
import { getNearbyStops, getRealTimeArrivals, getAlerts, SEPTA_ROUTES, SAMPLE_STOPS } from '@/lib/septa-api';
...
export default function HomePage() {
  ...
  const fetchNearbyDepartures = useCallback(async (showLoading = true) => {
    if (!location) return;
    ...
    try {
      ...
      await Promise.all(
        nearbyStops.slice(0, 5).map(async (stop) => {
          ...
          if (response.data) {
            response.data.slice(0, 3).forEach((arrival) => {
      ...
      allDepartures.sort((a, b) => a.arrival.minutesUntilArrival - b.arrival.minutesUntilArrival);
  ...
  useEffect(() => {
    async function fetchAlerts() {
      ...
      if (response.data) {
        setAlerts(response.data.filter(a => a.severity === 'severe').slice(0, 2));
      }
    }
    fetchAlerts();
  }, []);
  ...
  const showLocationLoading = permission === 'granted' && isLoadingLocation && !location;
        ...
        <div className="max-w-lg mx-auto">
          ...
          <Link href="/routes">
            <div className="input flex items-center gap-3 cursor-pointer hover:border-septa-blue transition-colors">
              ...
              <span className="text-text-muted">Search routes, stops, or places...</span>
            </div>
          </Link>
        </div>
      </header>
              ...
              <div className="flex items-center gap-3 p-3 rounded-xl bg-urgent/10 border border-urgent/20">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-urgent">
                    {severeAlerts.length} service alert{severeAlerts.length > 1 ? 's' : ''}
                  </p>
      ...
      <main className="px-4 pb-24">
        <div className="max-w-lg mx-auto">
          ...
          {hasFavorites && (
            <div className="flex gap-2 mb-4 p-1 bg-bg-secondary rounded-xl">
              <button
                ...
                className={`flex-1 tab ${activeTab === 'nearby' ? 'tab-active' : ''}`}
              >
              ...
              <button
                ...
                className={`flex-1 tab ${activeTab === 'favorites' ? 'tab-active' : ''}`}
              >
          ...
          <section>
            ...
            {!location && permission !== 'granted' && permission !== 'denied' && !isLoadingLocation && activeTab === 'nearby' && (
              <div className="card p-8 text-center">
                ...
                <p className="text-sm text-text-secondary mb-6">
                  Enable location to see real-time departures from nearby stops
                </p>
                <button
            ...
            {permission === 'denied' && activeTab === 'nearby' && (
              <div className="card p-8 text-center">
                ...
                <p className="text-sm text-text-secondary">
                  Enable location in your browser settings to see nearby departures
                </p>
              </div>
            )}

            {/* Loading State */}
            {isLoadingDepartures && !showLocationLoading && activeTab === 'nearby' && (
            ...
            {/* Departures List */}
            {!isLoadingDepartures && location && activeTab === 'nearby' && hasInitialized && (
              <div className="space-y-3">
                {departures.length > 0 ? (
                    ...
                    return (
                      <Link key={fav.stopId} href={`/stop/${fav.stopId}`}>
                        <div className="card p-4 flex items-center gap-4 hover:bg-bg-highlight transition-colors">
                          ...
                          <div className="flex-1 min-w-0">
                            <p className="font-semibold text-text-primary truncate">{fav.stopName}</p>
                            <p className="text-sm text-text-muted">Stop #{fav.stopId}</p>
                          </div>
          ...
          {activeTab === 'nearby' && (
            <section className="mt-8">
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-lg font-bold text-text-primary">Quick Routes</h2>
                <Link href="/routes" className="text-sm text-septa-blue font-medium">
                  View all
                </Link>
              </div>
                ...
                {['MFL', 'BSL', '17', '23', '42', 'AIR'].map((routeId) => {
                  ...
                  return (
                    <Link key={routeId} href={`/route/${routeId}`}>
                      <div className={`${getColor()} px-4 py-3 rounded-xl min-w-[80px] text-center transition-transform hover:scale-105`}>
                        <span className="font-mono font-bold text-white text-lg">{route.routeShortName}</span>
                        <p className="text-xs text-white/70 mt-1 truncate max-w-[80px]">
                          {route.routeType === 'subway' ? route.routeLongName.split(' ')[0] : route.routeLongName.split('-')[0]}
                        </p>
                      </div>
                    </Link>
                  );
